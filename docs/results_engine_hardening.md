...
After = current `main` after (flat id map, hierarchical bitmap, input
...
- Core pinning: in-process `SetThreadAffinityMask` to logical core 0 (no
  `taskset`/`lscpu` on Windows). Cannot control the P/E logical core mapping.
- Default balanced power plan.Turbo/throttling state could not be fixed on this machine
...
The tail moved the most, as expected: The flat map removed the per-insert
`unordered_map` node allocation, and the bitmap removed the O(gap)
...
**Note:** this is a shared, thermally-constrained, hybrid-core Windows
laptop with no fixed frequency governor and no true core isolation — not a
pinned bare-metal Linux machine. Absolute nanosecond
figures are directional, not as a clean lower bound; the max
...
before/after **comparison**, is run on the same machine under the same
conditions with the same harness, so it is comparable, and matches the expected improvement due to
flat-map and bitmap allocation/complexity.
